...
class map_bst
{
  ...
  public:
    ...
    void dfs(node* n, std::vector<ValueT> &vec) {
        if (!n) return;
        dfs(n->left, vec);
  ...
  void insert_recur(node* &n, std::vector<ValueT> &vec, int l, int r, node* par) {
    if (l > r) return;
    int mid = (l+r)/2;
    n = new node(vec[mid], NULL, NULL, par);
    insert_recur(n->left, vec, l, mid - 1, n);
    insert_recur(n->right, vec, mid + 1, r, n);
  }

  int get_balance(node *n, int height = 0) {
    if (!n) return -1;
    int lh = get_balance(n->left, height + 1);
    int rh = get_balance(n->right, height + 1);
    std::cout << n->data.first << " = " << std::abs(lh - rh) << "\n"; 
    return max(lh, rh) + 1;
  }
  ...
  void get_balance() {
    ...
    void make_balance() {
        ...
        clear();
        mSize = vec.size();
        insert_recur(mRoot, vec, 0, (int)vec.size() - 1, NULL);
    }
};
