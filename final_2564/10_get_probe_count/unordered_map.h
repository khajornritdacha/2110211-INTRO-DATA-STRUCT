...
class unordered_map
{
  ...
  public:
    ...
    size_t get_probe_count(KeyT &k) {
        size_t home = hash_to_bucket(k);
        size_t cur = home;
        size_t cnt = 0;
        std::cout << "bucket count: " << bucket_count() << "\n";
        while (!mBuckets[cur].empty() and !mEqual(k, mBuckets[cur].value.first)) cur = mNextAddress(home, ++cnt, bucket_count());
        return cnt;
    }
